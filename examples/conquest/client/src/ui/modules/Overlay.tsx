...
export const Achievements = ({ creations, completions, playerId }: { creations: AchievementCreation[], completions: AchievementCompletion[], playerId: string }) => {
  const sortedCreations = creations.sort((a: AchievementCreation, b: AchievementCreation) => parseInt(a.id.toString(10).slice(1)) - parseInt(b.id.toString(10).slice(1)));
  return (
    <div className="flex flex-col gap-8 items-center">
      <h1 className="text-4xl text-center">Achievements</h1>
      <div className="flex gap-4">
        {sortedCreations.map((creation: any) => (
          <Achievement key={creation.id} creation={creation} completions={completions} playerId={playerId} />
        ))}
      </div>
    </div>
  );
...
export const Achievement = ({ creation, completions, playerId }: { creation: AchievementCreation, completions: AchievementCompletion[], playerId: string }) => {
  const filteredCompletions = completions.filter((completion) => completion.id === creation.id && completion.player_id === BigInt(playerId));
